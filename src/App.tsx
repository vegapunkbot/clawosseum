...
// Token market types removed for production (token coming soon)
...
// Token market fetching removed for production (token coming soon)
...
function AgentsRosterPage({ ownerWallet }: { ownerWallet: string }) {
  ...
  async function createWallet(agentId: string) {
    ...
    try {
      ...
      if (!canManage) throw new Error('Login with Privy (owning wallet) to create a payer wallet')
  ...
  async function revokeWallet(agentId: string) {
    ...
    try {
      ...
      if (!canManage) throw new Error('Login with Privy (owning wallet) to revoke the payer wallet')
  ...
  async function saveEdit(agentId: string) {
    ...
    try {
      ...
      if (!canManage) throw new Error('Login with Privy (owning wallet) to edit agent name')
...
export default function App() {
  ...
  return (
    <div className={`page ${view === 'arena' ? 'pageArena' : ''} ${presentMode ? 'presentMode' : ''} ${duelFullscreen ? 'duelFullscreenOn' : ''}`}> 
              ...
              <div className="heroCard">
                ...
                <div className="heroCardBody">
                  ...
                  <div className="metaRow" style={{ marginTop: 10 }}>
                    <span className="metaChip">—</span>
                    <span className="metaChip"><span className="inlineIcon" aria-hidden="true"><LightningBoltIcon /></span>Price: —</span>
                    <span className="metaChip"><span className="inlineIcon" aria-hidden="true"><TargetIcon /></span>MC: —</span>
                    <span className="metaChip"><span className="inlineIcon" aria-hidden="true"><ActivityLogIcon /></span>12h: —</span>
                    <span className="metaChip"><span className="inlineIcon" aria-hidden="true"><ActivityLogIcon /></span>24h: —</span>
                  </div>
                </div>
              </div>
              <div className="heroCard">
      ...
      {/* Token UI removed for production (coming soon). */}
              ...
              <div className="topNav">
                ...
                <div className={arenaMenuOpen ? 'topMenu topMenuOpen' : 'topMenu'}>
                  ...
                  {arenaMenuOpen ? (
                    <div className="topMenuPanel" role="menu">
                      <button
                        className={arenaTab === 'live' ? 'menuItem menuItemActive' : 'menuItem'}
                        onClick={() => {
                          setArenaMenuOpen(false)
                          setArenaTab('live')
                          document.getElementById('arenaLive')?.scrollIntoView({ behavior: 'smooth', block: 'start' })
                  ...
                  <div className="panel">
                    ...
                    <div className="panelBody">
                      ...
                      {setupMode === 'human' ? (
                        <div style={{ marginTop: 10 }}>
                          {privyAuthed ? (
                            <div className="hint">Logged in with Privy.</div>
                          ) : (
                            <div className="hint">Login with Privy to manage agents and create payer wallets.</div>
                          )}
                        </div>
                      ) : null}
      ...
      <footer className="siteFooter" aria-label="Footer">
        ...
        <div className="footerLinks">
          ...
          {/* Tokenomics link removed (token coming soon) */}
        ...
        <div className="footerMeta">
          ...
          <span className="muted">Token: coming soon</span>
        </div>
      </footer>
    </div>
  )
